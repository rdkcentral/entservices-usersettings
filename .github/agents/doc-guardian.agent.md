...
3. Create a documentation PR from the current Copilot branch
...
3. Get current repository information:
   - Run: `gh repo view --json nameWithOwner,owner,name --jq '.nameWithOwner'`
   - Store the result as `REPO_FULL_NAME` (e.g. `rdkcentral/entservices-usersettings`)
   - Extract owner: `gh repo view --json owner --jq '.owner.login'` → store as `REPO_OWNER`
   - Extract name: `gh repo view --json name --jq '.name'` → store as `REPO_NAME`

4. Configure git credentials and push commits:
   - **Token Options:** The agent will use `DOC_COPILOT_PAT` if available in repository secrets, otherwise fall back to the default GitHub token
...
5. Create documentation PR in the same repository:
   ...
   - PR Body should include:
     ...
     - Link to original PR that triggered this
     - Any areas needing SME review
     - The line: `Doc-Guardian-Issue: #<issue_number>`
   
   - If `gh pr create` fails with an error (e.g. 403 Forbidden, authentication error), capture the exact error and post a comment on the originating issue explaining:
     - The exact error returned
     - That the branch is available at `$CURRENT_BRANCH`
     - That this may indicate insufficient permissions
     - Instructions for the human to manually raise the PR: `gh pr create --head "$CURRENT_BRANCH" --base develop`
     - Then stop

6. Link documentation PR to original PR (if available):
   - Add a comment to the original PR mentioning the documentation PR
   - Reference the originating issue in the documentation PR body
